...
function printPicture(data) {
  if (data.media_type === 'video') {
    ...
    document.getElementById('response-apod').append(iframe);
  } else {
    let img = document.createElement('img');
    img.src = data.url;
    img.alt = `${data.media_type} of ${data.title}`;
    document.getElementById('response-apod').append(img);
  }
}
...
function printWiki(data) {
  ...
  document.getElementById("response-wiki").append(p);
  let img = document.createElement('img');
  img.setAttribute('src', data.selected[randEvent].pages[0].originalimage.source);
  img.setAttribute('width', '400px');
  img.setAttribute('height', 'auto');
  document.getElementById('response-wiki').append(img);
}
...
function handleSubmission(e) {
  e.preventDefault();
  //handle error
  document.getElementById('response-apod').innerHTML = null;
  document.getElementById('response-wiki').innerHTML = null;
...
function printNotValidDate(userDate) {
  document.getElementById('response-error').innerText = `${userDate} is not a valid date`;
}
